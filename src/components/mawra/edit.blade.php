...
@php $que = DB::table('fmt_mawra_ques')->where('id', $message)->first(); @endphp
<div class="fixed z-10 inset-0 overflow-y-auto hidden" id="modalMAWRA{{$que->id}}">
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-xs text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <a onclick="closeModalMAWRA({{$message}})" style="z-index: 999999999999999999999999; position: relative; background:#000; color:#fff; " class="p-2 w-8 h-8 rounded-full absolute right-0 -top-10 -mr-2 -mt-2 z-40" href="javascript:void(0);">close</a>
                <form action="{{route('fmt.mawra.update', $que->id)}}" method="post" enctype="multipart/form-data">
                    @if ($errors ?? '')
                        <div class="my-4">
                    ...
                    @endif
                    @csrf
                    <div class="text-xl">Edit MAWRA Question {{$message}}</div>
                    ...
                    @php $answers = DB::table('fmt_mawra_ans')->where('question_id', $que->id)->pluck('id'); @endphp
                    @foreach ($answers as $ans)
                        @php $answer = DB::table('fmt_mawra_ans')->where('id', $ans)->first(); @endphp
                        {{--  --}}
                        <div class="flex flex-wrap my-4 border-2 border-gray-400" id="ans1">
                            <div class="w-1/3 px-2">
                                ...
                                <input class="my-2 border border-gray-600 p-2 w-full rounded-lg" type="text" name="answer{{$answer->id}}" placeholder="Answer" value="{{$answer->answer}}">
                            </div>
                            <div class="w-1/3 px-2">
                                <label class="w-full" for="">Arrange</label>
                                <input class="my-2 border border-gray-600 p-2 w-full rounded-lg" type="text" name="arrange{{$answer->id}}" placeholder="Arrange" value="{{$answer->arrange}}">
                            </div>
                            <div class="w-1/3">
